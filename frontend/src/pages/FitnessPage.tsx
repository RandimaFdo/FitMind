...
import { CalendarCheck2, MapPin, Plus, Users, Zap } from 'lucide-react'
...
export function FitnessPage() {
  ...
  const bookingGymInstructors = useMemo(
    () => instructors.filter((instructor) => instructor.gymId === bookingGymId),
    [bookingGymId, instructors],
  )
    ...
    <div className="space-y-10">
        ...
        <div className="mt-4 flex flex-wrap items-end justify-between gap-6">
          <div className="max-w-2xl space-y-4">
            ...
            <p className="text-base text-white/80">
              Curate weekly training flows, discover vetted gyms, and sync instructors with every appointment—all inside one structured workspace.
            </p>
          </div>
          <button
            type="button"
      ...
      <section className="space-y-6 rounded-3xl border border-white/5 bg-slate-900/60 p-6">
        <header className="flex flex-wrap items-center gap-4">
          ...
          <button
            type="button"
            onClick={() => document.getElementById('add-gym')?.scrollIntoView({ behavior: 'smooth' })}
      ...
      <section className="space-y-6 rounded-3xl border border-white/5 bg-slate-900/60 p-6">
        <header className="flex flex-wrap items-center gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs uppercase tracking-[0.4em] text-brand-aqua">Assign instructors</p>
              <h2 className="text-2xl font-semibold text-white">Bookings & instructor routing</h2>
            </div>
          </div>
        </header>
      ...
      <section className="space-y-6 rounded-3xl border border-white/5 bg-slate-900/70 p-6">
        <header className="flex items-center gap-3">
          <Users className="h-10 w-10 rounded-2xl border border-white/10 p-2 text-brand-aqua" />
          <div>
            <p className="text-xs uppercase tracking-[0.4em] text-brand-aqua">Instructor roster</p>
            <h2 className="text-2xl font-semibold text-white">Who's active</h2>
          </div>
        </header>
        <div className="grid gap-4 md:grid-cols-2">
          {instructors.map((instructor) => {
            const gym = gyms.find((item) => item.id === instructor.gymId)
            return (
              <article key={instructor.id} className="rounded-2xl border border-white/10 bg-slate-900/50 p-4">
          ...
          {instructors.length === 0 && (
            <p className="text-sm text-slate-400">No instructors yet. Add them from each gym detail view.</p>
          )}
        </div>
      </section>

      <section id="add-gym" className="space-y-6 rounded-3xl border border-white/5 bg-slate-900/70 p-6">
        <header className="flex items-center gap-3">
        ...
        <p className="text-sm text-slate-300">
          Keep your network fresh by logging new gyms, pop-up studios, or speciality labs that match your training culture. Each new entry becomes immediately bookable.
        </p>
        <button
          type="button"
          onClick={() => setIsAddGymOpen(true)}
      ...
      <AppointmentModal
        ...
        instructors={bookingGymInstructors}
        gymName={bookingGym?.name ?? 'Gym'}
        defaultInstructorId={bookingGymInstructors[0]?.id}
      />
    </div>
  )
}
